...
public class TreasureIsland {
    public int minRouts(char[][] input){
        ...
        Point startPoint = new Point(0, 0, Color.GREY, 1);
        queue.add(new Point(0, 0, Color.GREY, 1));
        matrix[0][0] = startPoint;
        while(!queue.isEmpty()) {
            Point currPoint = queue.remove();
            if(input[currPoint.x][currPoint.y] == 'X') {
                return currPoint.step;
            }
            matrix[currPoint.x][currPoint.y].color = Color.BLACK;
//            ++ res;
            if(currPoint.x + 1 < maxRow
                    && input[currPoint.x + 1][currPoint.y] == '0'
                    && matrix[currPoint.x + 1][currPoint.y].color == Color.WHITE) {
                queue.add(new Point(currPoint.x + 1, currPoint.y, Color.GREY, currPoint.step + 1));
            }
            // if() row -1
            // if() column + 1
            // if() column - 1
...
class Point {
    ...
    public Color color;
    public int step;
    Point(int x, int y, Color color, int step) {
        this.x = x;
        this.y = y;
        this.color = color;
        this.step = step;
